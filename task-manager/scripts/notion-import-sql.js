#!/usr/bin/env node
// Notion 20_ツクツク → D1 直接インポート用SQLファイル生成
// Usage: node scripts/notion-import-sql.js
// 出力: scripts/notion-import.sql
// 実行: npx wrangler d1 execute task-manager-db --remote --file scripts/notion-import.sql
...
const OUT_FILE = path.join(__dirname, 'notion-import.sql');
...
const lines = [];
lines.push('-- Notion 20_ツクツク インポート SQL');
...
raw.forEach(e => {
  ...
  const tags = Array.isArray(e.tags) ? JSON.stringify(e.tags) : (e.tags || '[]');
  const truncated = body.length >= MAX_BODY ? ' [省略あり]' : '';
  console.log(`  [${e.id.slice(-8)}] ${e.title} (${body.length}文字${truncated})`);
  ...
  lines.push(
    ...
    `${sqlStr(tags)}, ` +
    `NULL, ` +
    `${e.parent_id ? sqlStr(e.parent_id) : 'NULL'}, ` +
    `datetime('now'), ` +
    `datetime('now')` +
    `);`
  );
...
console.log(`\nSQL出力完了: ${OUT_FILE}`);
console.log('\n次のコマンドで本番D1に書き込み:');
console.log('npx wrangler d1 execute task-manager-db --remote --file scripts/notion-import.sql');
