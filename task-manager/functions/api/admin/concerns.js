...
export async function onRequestGet(context) {
  ...
  const result = await env.DB.prepare(
    'SELECT id, body, urgency, category, status, resolution, created_at, updated_at, resolved_at, auto_resolved FROM customer_concerns WHERE customer_id = ? ORDER BY created_at DESC'
  ).bind(customerId).all();
