# PROJECT.md（Homepage / Static Site）

## 0. 前提（最優先）
- ルートの SHARED.md（事業OS/共通ルール）に必ず従う。
...
- デプロイコマンド（ホームページディレクトリ内でBash実行）：
  ```bash
  cd "H:\共有ドライブ\60_Web編集用\kigyonooishasan\10_ホームページ" && npx wrangler@3 pages deploy . --project-name kigyonooishasan-hp --branch main --commit-dirty=true
  ```
